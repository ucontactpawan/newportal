...
// Build query
$query = "SELECT a.*, e.name as employee_name, s.shift_start as schedule_time 
          FROM attendance_logs a 
          JOIN employees e ON a.employee_id = e.id 
          LEFT JOIN schedules s ON a.employee_id = s.employee_id 
          AND DAYNAME(a.date) = s.day_of_week 
          WHERE 1=1";
                ...
                <tbody>
                    <?php while ($row = mysqli_fetch_assoc($result)) { ?>
                        <tr>
                            ...
                            <td><?php echo $row['schedule_time'] ? date('h:i A', strtotime($row['schedule_time'])) : '-'; ?></td>
                            <td>
                                <div data-entry-time="<?php echo $row['check_in']; ?>"
                                    data-schedule-time="<?php echo $row['schedule_time']; ?>">
                                </div>
                            </td>
                        </tr>
                    <?php } ?>
